...
final class TwigCompleteCheckRule implements Rule
{
    ...
    public function processNode(Node $node, Scope $scope) : array
    {
        ...
        foreach ($renderTemplatesWithParameters as $renderTemplateWithParameter) {
            ...
            $variablesAndTypes = $this->templateVariableTypesResolver->resolveArray($renderTemplateWithParameter->getParametersArray(), $scope);
            $currentRuleErrors = $this->processTemplateFilePath($renderTemplateWithParameter->getTemplateFilePath(), $variablesAndTypes, $scope, $node->getLine());
...
CODE_SAMPLE
     ...
     * @return RuleError[]
     */
    private function processTemplateFilePath(string $templateFilePath, array $variablesAndTypes, Scope $scope, int $phpLine) : array
    {
