...
use RevealPrefix20220708\Nette\PhpGenerator as Php;
/**
...
class Blueprint
{
    ...
    private function printType(?string $type, bool $nullable, ?Php\PhpNamespace $namespace) : string
    {
        ...
        if ($nullable && \strcasecmp($type, 'mixed')) {
            $type = \strpos($type, '|') !== \false ? $type . '|null' : '?' . $type;
        }
        return $type;
    }
    /**
    ...
    public function printParameters($function, ?Php\PhpNamespace $namespace = null) : string
    {
        ...
        foreach ($list as $param) {
            ...
            $params[] = \ltrim($this->printType($param->getType(), $param->isNullable(), $namespace) . ' ') . ($param->isReference() ? '&' : '') . ($variadic ? '...' : '') . '$' . $param->getName() . ($param->hasDefaultValue() && !$variadic ? ' = ' . \var_export($param->getDefaultValue(), \true) : '');
        }
    ...
    {
        echo '<pre><code class="language-', \htmlspecialchars($lang), '">', \htmlspecialchars($code), "</code></pre>\n";
    }
}
