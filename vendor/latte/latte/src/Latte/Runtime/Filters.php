...
use RevealPrefix20220708\Latte\RuntimeException;
use RevealPrefix20220708\Nette;
...
class Filters
{
    ...
    {
        if (!is_string($new) || !\preg_match('~' . Latte\Parser::RE_TAG_NAME . '$~DA', $new)) {
            throw new Latte\RuntimeException('Invalid tag name ' . \var_export($new, \true));
        }
        $new = \strtolower($new);
        if ($new === 'style' || $new === 'script' || isset(Latte\Helpers::$emptyElements[\strtolower($orig)]) !== isset(Latte\Helpers::$emptyElements[$new])) {
