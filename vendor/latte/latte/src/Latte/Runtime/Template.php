...
namespace RevealPrefix20220708\Latte\Runtime;

use RevealPrefix20220708\Latte;
use RevealPrefix20220708\Latte\Engine;
use RevealPrefix20220708\Latte\Policy;
/**
 * Template.
 */
class Template
{
    ...
    public function renderBlock(string $name, array $params, $mod = null, $layer = null) : void
    {
        ...
        if (!$block) {
            $hint = ($t = Latte\Helpers::getSuggestion($this->getBlockNames($layer), $name)) ? ", did you mean '{$t}'?" : '.';
    ...
    protected function addBlock(string $name, string $contentType, array $functions, $layer = null) : void
    {
        ...
        if ($block->contentType === null) {
            ...
            throw new Latte\RuntimeException(\sprintf("Overridden block {$name} with content type %s by incompatible type %s.", \strtoupper($contentType), \strtoupper($block->contentType)));
        }
    ...
    private function filter(callable $function, $mod, string $contentType, string $name) : void
    {
        if ($mod === null || $mod === $contentType) {
            ...
            throw new Latte\RuntimeException(\sprintf("Including {$name} with content type %s into incompatible type %s.", \strtoupper($contentType), \strtoupper($mod)));
        }
    }
    /**
