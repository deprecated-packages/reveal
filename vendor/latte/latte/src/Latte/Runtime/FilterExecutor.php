...
class FilterExecutor
{
    ...
    public function filterContent(string $name, FilterInfo $info, ...$args)
    {
        ...
        if ($info->contentType !== Engine::CONTENT_TEXT) {
            throw new Latte\RuntimeException("Filter |{$name} is called with incompatible content type " . \strtoupper($info->contentType) . ($info->contentType === Engine::CONTENT_HTML ? ', try to prepend |stripHtml.' : '.'));
        }
        $res = ($this->{$name})(...$args);
