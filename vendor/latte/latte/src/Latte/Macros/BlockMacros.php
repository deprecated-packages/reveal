...
use RevealPrefix20220707\Latte\Runtime\Block;
use RevealPrefix20220707\Latte\Runtime\SnippetDriver;
...
class BlockMacros extends MacroSet
{
    ...
    public function macroIfset(MacroNode $node, PhpWriter $writer)
    {
        ...
        while ([$name, $block] = $node->tokenizer->fetchWordWithModifier(['block', '#'])) {
            $list[] = $block || \preg_match('~\\w[\\w-]*$~DA', $name) ? '$this->hasBlock(' . $writer->formatWord($name) . ')' : 'isset(' . $writer->formatArgs(new Latte\MacroTokens($name)) . ')';
        }
