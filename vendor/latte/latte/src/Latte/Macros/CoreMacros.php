...
namespace RevealPrefix20220707\Latte\Macros;

use RevealPrefix20220707\Latte;
use RevealPrefix20220707\Latte\CompileException;
use RevealPrefix20220707\Latte\Engine;
use RevealPrefix20220707\Latte\Helpers;
use RevealPrefix20220707\Latte\MacroNode;
use RevealPrefix20220707\Latte\PhpHelpers;
use RevealPrefix20220707\Latte\PhpWriter;
/**
...
class CoreMacros extends MacroSet
{
    ...
    public function finalize()
    {
        ...
        if ($this->overwrittenVars) {
            ...
            $code .= 'foreach (array_intersect_key(' . Latte\PhpHelpers::dump($vars) . ', $this->params) as $ʟ_v => $ʟ_l) { ' . 'trigger_error("Variable \\$$ʟ_v overwritten in foreach on line $ʟ_l"); } ';
        }
    ...
    {
        $node->validate(\false);
        $node->openingCode = $writer->write(\in_array($node->context[0], [Engine::CONTENT_HTML, Engine::CONTENT_XHTML], \true) ? "<?php ob_start('Latte\\Runtime\\Filters::spacelessHtmlHandler', 4096) %node.line; try { ?>" : "<?php ob_start('Latte\\Runtime\\Filters::spacelessText', 4096) %node.line; try { ?>");
    ...
    public function macroTagEnd(MacroNode $node, PhpWriter $writer) : void
    {
        ...
        $node->content = \preg_replace('~^(\\s*<)' . Latte\Parser::RE_TAG_NAME . '~', "\$1<?php echo \$ʟ_tag[{$node->htmlNode->data->id}]; ?>\n", $node->content);
        $node->content = \preg_replace('~</' . Latte\Parser::RE_TAG_NAME . '(\\s*>\\s*)$~', "</<?php echo \$ʟ_tag[{$node->htmlNode->data->id}]; ?>\n\$1", $node->content);
    }
    /**
     * {dump ...}
     */
